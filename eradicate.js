...
fbLink = $("<a href='javascript:;'>News Feed Eradicator Info</a>")
        ...
        .on('click', function () {
            ...
            if (window.chrome !== undefined) {
                // Chrome extension
                infoPanel.load(chrome.extension.getURL(url), handleClose);
            } else {
                // Firefox extension
...
setInterval(function () {
    ...
    chrome.storage.sync.get({
        ...
        switch (new Date().getDay()) {
            case 0:
                //  day = "Sunday";
                if (items.sunday == false) {
                    deleteNewsfeed();
                }
                break;
            case 1:
                // day = "Monday";
                if (items.monday == false) {
                    deleteNewsfeed();
                }
                break;
            case 2:
                //day = "Tuesday";
                if (items.tuesday == false) {
                    deleteNewsfeed();
                }
                break;
            case 3:
                // day = "Wednesday";
                if (items.wednesday == false) {
                    deleteNewsfeed();
                }
                break;
            case 4:
                // day = "Thursday";
                if (items.thursday == false) {
                    deleteNewsfeed();
                }
                break;
            case 5:
                //  day = "Friday";
                if (items.friday == false) {
                    deleteNewsfeed();
                }
                break;
            case 6:
                //  day = "Saturday";
                if (items.saturday == false) {
                    deleteNewsfeed();
                }
                break;
        }
